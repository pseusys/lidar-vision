# Three Horizons: a LiDAR person detector that remembers

Person detection from knee-height 2D LiDAR on mobile robots · FROG benchmark · **first full result, September 2026**
...
1. **Input**: 11 pose-invariant features per beam.
2. **Calibration network** (1.18 M parameters): a 1D ConvNeXt U-Net over 720 beams with causal temporal convolutions, fine per beam (3 frames, 0.08 s) and coarse per sector at the bottleneck (5 frames, 1.2 s), their caches re-aligned by odometry. A DROW vote head yields 64 candidates, sub-threshold ones included.
3. **Object memory** (0.31 M parameters): 256 slots in world coordinates; cross-attention association, slot self-attention, and a selective diagonal recurrence (Mamba-style, Δt-aware, decay times 1-60 s). It rescores candidates, coasts briefly missed people, and can render predicted slot positions back into the calibration network.
...
| our calibration network alone | ~1.3 s | 76.9% | 58.4% | 87.3% | 1.91 |
| SORT on its candidates *(best setting)* | unbounded, hand-tuned | 76.7% | 59.1% | 86.8% | 1.84 |
| **+ object memory** | **seconds to a minute** | **80.2%** | 66.7% | 84.7% | 1.30 |

Published rows are the papers' own figures; ours are measured here over every annotated frame of the test recording.

- **The memory adds 3.3 points** over its own input and 4.6 over DR-SPAAM's published figure — ours measured here against a published number, one seed.
- **The gain is the learned memory, not temporal filtering:** a SORT tracker on the same candidates *lowers* AP at every setting, trading recall for precision about one for one.
- **History is used:** shuffling the order of past scans costs the calibration network 1.3 points.
- **Capacity is not the limit:** 64 slots score within 0.1 points of 256.
- **Still open:** training both stages jointly, with the memory feeding back into the detector, reached only 78.9% and is under investigation; and a third of reported detections are still false.
...
Measured while streaming scans one at a time on one consumer GPU (Radeon RX 9060 XT, ROCm), the calibration network plus candidate decoding runs at **~10.4 ms per frame** — against 26.2 scans per second from this sensor. That figure includes a Python-side decoding loop and excludes the memory; a proper latency benchmark of the whole detector is still to be run.
