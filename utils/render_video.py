...
def _load_nn_runner(det_name: str, weights_path: Path, cfg, device: str):
    ...
    def _runner(dataset, seq, det):
        ...
        if input_mode == "raw_scan":
            r = raw_scan(scans_hist)
            x_in = torch.from_numpy(r.transpose(1, 0, 2)).to(device)
        else:
...
def _parse():
    ...
    p.add_argument("--fullscan-tcn",       metavar="WEIGHTS", default=None)
...
def main():
    ...
    nn_map = [
        ...
        ("fullscan_tcn",  args.fullscan_tcn),
    ]
